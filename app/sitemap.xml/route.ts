...
export async function GET() {
  ...
  const latestVideo = newestEntry(data.videos);
  const latestVideoChange = latestDate(latestVideo, data.videoSettingsUpdatedAt);
  ...
  const urls = [
    ...
    xmlUrl("/video", latestVideoChange),
    xmlUrl("/video/work", latestVideo),
    ...
    ...data.projects.map((entry) => xmlUrl(`/projects/${entry.slug}`, entry.updatedAt)),
    ...data.labs.map((entry) => xmlUrl(`/labs/${entry.slug}`, entry.updatedAt)),
    ...data.articles.map((entry) => xmlUrl(`/journal/${entry.slug}`, entry.updatedAt)),
    ...data.videos.map((entry) => xmlUrl(`/video/${entry.slug}`, entry.updatedAt)),
