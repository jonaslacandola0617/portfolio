...
import { getHomepageShowcaseProjectIds } from "@/lib/db/queries/homepage";
...
export default async function HomePage() {
  const [projects, articles, labs, settings, about, showcaseProjectIds] = await Promise.all([
    getAllProjects(),
    getAllArticles(),
    getAllLabs(),
    getSiteSettings(),
    getAboutPage(),
    getHomepageShowcaseProjectIds(),
  ]);

  const projectById = new Map(projects.map((project) => [project.recordId, project]));
  const primaryProject = showcaseProjectIds[0] ? projectById.get(showcaseProjectIds[0]) : undefined;
  const secondaryProject = showcaseProjectIds[1] ? projectById.get(showcaseProjectIds[1]) : undefined;
