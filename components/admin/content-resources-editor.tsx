...
import { updateArticleResourcesAction } from "@/app/admin/(dashboard)/journal/actions";
...
interface ContentResourcesEditorProps {
  contentType: "lab" | "article";
  recordId: string;
  media: AdminMediaItem[];
  initialResources: Resource[];
}

export function ContentResourcesEditor({
  contentType,
  recordId,
  media,
  initialResources,
}: ContentResourcesEditorProps) {
  ...
  const contentLabel = contentType === "article" ? "journal entry" : "lab";
  ...
  const save = () => startTransition(async () => {
    const orderedResources = resources.map((resource, sortOrder) => ({ ...resource, sortOrder }));
    const result = contentType === "article"
      ? await updateArticleResourcesAction({ articleId: recordId, resources: orderedResources })
      : await updateLabResourcesAction({ labId: recordId, resources: orderedResources });

    if (result.success) {
      setMessage(null);
      success(result.message ?? "Resources saved.", {
        id: `${contentType}-resources:${recordId}`,
      });
    } else {
  ...
  const uploadResource = async (file: File) => {
    ...
    try {
      ...
      success("Resource uploaded and added.", {
        id: `${contentType}-resource-upload:${created.id}`,
      });
    } catch (error) {
      setMessage({
        success: false,
    ...
    <section className="space-y-3">
      <div>
        ...
        <p className="text-xs text-text-dim">
          Attach downloadable files to this {contentLabel}. Removing an association does not delete the Media Library file.
        </p>
      </div>
      ...
      <div className="space-y-2">
        {resources.map((resource, index) => {
          ...
          return (
            <div key={`${resource.mediaId}-${index}`} className="border border-border bg-surface px-3 py-3">
              ...
              <div className="space-y-3">
                <div>
                  <Label htmlFor={`${contentType}-resource-label-${index}`}>Display name</Label>
                  <Input id={`${contentType}-resource-label-${index}`} value={resource.label} onChange={(event) => setResources((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, label: event.target.value } : entry))} />
                </div>
                <div>
                  <Label htmlFor={`${contentType}-resource-description-${index}`}>Description</Label>
                  <Input id={`${contentType}-resource-description-${index}`} value={resource.description} onChange={(event) => setResources((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, description: event.target.value } : entry))} />
                </div>
              </div>
            </div>
          );
