...
const textareaClassName =
  "w-full border-0 border-b border-border-strong bg-transparent px-0 py-2.5 text-sm text-text outline-none focus:border-cobalt";
...
export function AboutForm({ about }: { about: AboutPageValues }) {
  ...
  return (
    <form action={action} className="admin-control-about-form">
      ...
      <section className="admin-control-about-profile">
        <div>
          <span className="label mb-2 block">PROFILE / IMAGE</span>
          <h2>Public portrait</h2>
          <p>This image is used only for the About presentation and stays separate from the Media Library.</p>
        </div>
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-2">
            ...
            <p className="max-w-xs text-[10px] leading-5 text-muted">JPEG, PNG, or WebP. Up to 5 MB.</p>
          </div>
        </div>
        {profileError && <p className="mt-2 text-xs text-vermilion">{profileError}</p>}
      </section>

      <div className="admin-control-about-fields">
        ...
        <div>
          ...
          <p className="mt-1 text-[10px] text-muted">One tag per line.</p>
        </div>
      ...
      <div className="admin-control-about-foot">
        <button type="submit" className="flex items-center gap-2 border border-border-strong bg-text px-4 py-2.5 text-sm font-medium text-surface">
          <Save size={13} /> Save About
        </button>
      </div>
    </form>
  );
