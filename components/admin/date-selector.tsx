...
export function DateSelector({
  ...
  const years = useMemo(
    () => Array.from({ length: currentYear - 1949 }, (_, index) => String(currentYear + 1 - index)),
    [currentYear]
  );
  const dayCount = year && month ? new Date(Date.UTC(Number(year), Number(month), 0)).getUTCDate() : 31;
  const safeDay = day && Number(day) <= dayCount ? day : "";
  const completeValue = year && month && safeDay ? `${year}-${month}-${safeDay}` : "";
  const hasPartialValue = Boolean(year || month || day) && !completeValue;
  const submittedValue = hasPartialValue ? "__incomplete_date__" : completeValue;
    ...
    <fieldset className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <legend className="text-sm font-medium leading-none text-foreground">
          {label}{optional && <span className="ml-1 font-normal text-muted-foreground">(optional)</span>}
        </legend>
      ...
      <p className="flex min-h-4 items-center gap-1.5 font-mono text-[0.68rem] text-muted-foreground">
        ...
        {hasPartialValue ? "Complete all three date fields" : completeValue || "No date selected"}
      </p>
    </fieldset>
  );
}
