...
interface ProjectFrontmatterRaw {
  title: string; slug: string; summary: string; category: string; difficulty: string;
  status: string; tags: string[]; technologies: string[]; skills: string[];
  estimatedTime: string; completionDate: string; githubUrl?: string;
  downloads?: { label: string; href: string; type: string }[];
  relatedCertification?: string;
}
interface LabFrontmatterRaw {
  title: string; slug: string; purpose: string; date: string; status: string;
  difficulty: string; tags: string[]; category: string;
}
interface ArticleFrontmatterRaw {
  title: string; slug: string; summary: string; date: string; tags: string[]; category: string;
}
...
async function seedProjects() {
  ...
  for (const filename of files) {
    ...
    const project = await prisma.project.upsert({
      where: { slug: fm.slug },
      create: {
        title: fm.title, slug: fm.slug, summary: fm.summary, content: toPrismaJson(validated),
        difficulty: fm.difficulty.toUpperCase() as never,
        progressStatus: fm.status.toUpperCase().replace(/-/g, "_") as never,
        publishStatus: "PUBLISHED", estimatedTime: fm.estimatedTime,
        completionDate: new Date(fm.completionDate), githubUrl: fm.githubUrl,
        technologies: fm.technologies ?? [], publishedAt: new Date(),
        category: { connectOrCreate: { where: { slug: slugify(fm.category) }, create: { name: fm.category, slug: slugify(fm.category) } } },
        tags: { connectOrCreate: fm.tags.map((tag) => ({ where: { slug: slugify(tag) }, create: { name: tag, slug: slugify(tag) } })) },
        skills: { connectOrCreate: (fm.skills ?? []).map((skill) => ({ where: { name: skill }, create: { name: skill, group: skillLookup.get(skill)?.group ?? "Ungrouped", level: skillLookup.get(skill)?.level ?? "practiced" } })) },
        downloads: { create: (fm.downloads ?? []).map((d) => ({ label: d.label, url: d.href, type: d.type })) },
      },
      update: {
        title: fm.title, summary: fm.summary, content: toPrismaJson(validated),
        difficulty: fm.difficulty.toUpperCase() as never,
        progressStatus: fm.status.toUpperCase().replace(/-/g, "_") as never,
        estimatedTime: fm.estimatedTime, completionDate: new Date(fm.completionDate),
        githubUrl: fm.githubUrl, technologies: fm.technologies ?? [],
      },
    });
...
async function seedLabs() {
  ...
  for (const filename of files) {
    ...
    const lab = await prisma.lab.upsert({
      where: { slug: fm.slug },
      create: {
        title: fm.title, slug: fm.slug, purpose: fm.purpose, content: toPrismaJson(validated),
        difficulty: fm.difficulty.toUpperCase() as never,
        progressStatus: fm.status.toUpperCase().replace(/-/g, "_") as never,
        publishStatus: "PUBLISHED", labDate: new Date(fm.date), publishedAt: new Date(),
        category: { connectOrCreate: { where: { slug: slugify(fm.category) }, create: { name: fm.category, slug: slugify(fm.category) } } },
        tags: { connectOrCreate: fm.tags.map((tag) => ({ where: { slug: slugify(tag) }, create: { name: tag, slug: slugify(tag) } })) },
      },
      update: {
        title: fm.title, purpose: fm.purpose, content: toPrismaJson(validated),
        difficulty: fm.difficulty.toUpperCase() as never,
        progressStatus: fm.status.toUpperCase().replace(/-/g, "_") as never,
...
async function seedArticles() {
  ...
  for (const filename of files) {
    ...
    const article = await prisma.article.upsert({
      ...
      create: {
        title: fm.title, slug: fm.slug, summary: fm.summary, content: toPrismaJson(validated),
        publishStatus: "PUBLISHED", date: new Date(fm.date), publishedAt: new Date(),
        category: { connectOrCreate: { where: { slug: slugify(fm.category) }, create: { name: fm.category, slug: slugify(fm.category) } } },
        tags: { connectOrCreate: fm.tags.map((tag) => ({ where: { slug: slugify(tag) }, create: { name: tag, slug: slugify(tag) } })) },
      },
      update: { title: fm.title, summary: fm.summary, content: toPrismaJson(validated), date: new Date(fm.date) },
    });
  ...
  for (const [sortOrder, cert] of certifications.entries()) {
    const record = await prisma.certificate.upsert({
      ...
      create: {
        ...
        skills: { connectOrCreate: cert.skills.map((skill) => ({ where: { name: skill }, create: { name: skill, group: skillLookup.get(skill)?.group ?? "Ungrouped", level: skillLookup.get(skill)?.level ?? "practiced" } })) },
      },
      update: {
        name: cert.name, issuer: cert.issuer, logo: cert.logo,
        progressStatus: "COMPLETED", progressLabel: "Completed", progressPercent: 100,
...
async function reconcileProjectCertificates() {
  ...
  for (const filename of files) {
    ...
    await prisma.project.update({ where: { slug: fm.slug }, data: { certificates: { connect: { slug: fm.relatedCertification } } } });
    console.log(`  ✓ ${fm.slug} ↔ ${fm.relatedCertification}`);
  }
}

async function seedSkills() {
  const allSkills = skillCategories.flatMap((c) => c.skills.map((s) => ({ ...s, group: c.category })));
...
async function seedSiteSettings() {
  await prisma.siteSettings.upsert({
    where: { id: "singleton" },
    create: {
      id: "singleton", name: siteConfig.name, role: siteConfig.role, tagline: siteConfig.tagline,
      email: siteConfig.email, githubUrl: siteConfig.social.github,
      linkedinUrl: siteConfig.social.linkedin, resumeUrl: siteConfig.resumeUrl,
