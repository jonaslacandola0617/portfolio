      ...
      create: {
        ...
        skills: {
          connectOrCreate: (fm.skills ?? []).map((skill) => ({
            ...
            create: {
              name: skill,
              group: skillLookup.get(skill)?.group ?? "Networking",
  ...
  for (const cert of certifications) {
    const record = await prisma.certificate.upsert({
      ...
      create: {
        ...
        issuer: cert.issuer,
        logo: cert.logo,
        progressStatus: cert.status.toUpperCase().replace(/-/g, "_") as never,
        publishStatus: "PUBLISHED",
        progressLabel: cert.progressLabel,
        progressPercent: cert.progressPercent,
        dateStarted: new Date(cert.dateStarted),
        dateCompleted: cert.dateCompleted ? new Date(cert.dateCompleted) : undefined,
        ...
        skills: {
          connectOrCreate: cert.skills.map((skill) => ({
            ...
            create: {
              name: skill,
              group: skillLookup.get(skill)?.group ?? "Cybersecurity",
              level: skillLookup.get(skill)?.level ?? "practiced",
            },
          })),
        },
